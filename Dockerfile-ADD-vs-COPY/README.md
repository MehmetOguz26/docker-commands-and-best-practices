...
Create a Dockerfile with ADD and COPY instructions to understand their differences in Docker image building.
...
# Example:
docker build -t dockerfile-add-vs-copy:v1 .

# Run Docker Container and Verify
docker run --name add-vs-copy -p 8080:80 -d dockerfile-add-vs-copy:v1

# List Static Files from Docker Container
docker exec -it add-vs-copy ls -lrta /usr/share/nginx/html
...
docker rm -f add-vs-copy

# Remove the Docker images
docker rmi mehmetoguz26/dockerfile-add-vs-copy:v1
docker rmi dockerfile-add-vs-copy:v1
