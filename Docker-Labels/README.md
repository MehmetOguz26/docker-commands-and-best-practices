...
Learn how to create a Docker image with labels, build it, inspect it, and push it to Docker Hub. This guide covers Dockerfile creation, adding labels, building images, and using the docker inspect command.
...
# Example:
docker build -t dockerfile-labels:v1 .
...
docker run --name dlabel -p 8080:80 -d dockerfile-labels:v1
...
# Example:
docker inspect --format='{{json .NetworkSettings}}' dlabel | jq
```
...
docker rmi dockerfile-labels:v1
